...
export function SavingsCalculatorSection() {
  ...
  return (
    <section id="calculator" className="relative bg-background dark:bg-black py-20">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <ScrollReveal delay={0.2}>
          <h2 className="text-2xl md:text-4xl font-mono text-foreground dark:text-white mb-4 tracking-wider text-center">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
          ...
          <ScrollReveal delay={0.3}>
            <div className="border border-foreground/20 dark:border-white/20 rounded-2xl p-6 md:p-8 backdrop-blur-sm bg-foreground/5 dark:bg-white/5 overflow-hidden">
              <div className="grid grid-cols-2 gap-2 mb-6">
                <button
                  ...
                  className={`w-full justify-center inline-flex items-center gap-2 px-3 py-2 rounded-md border text-xs sm:text-sm font-mono tracking-wide transition truncate
                    ${
                      profile === "business"
                        ? "border-foreground/40 dark:border-white/40 bg-foreground/10 dark:bg-white/10 text-foreground dark:text-white"
                        : "border-foreground/20 dark:border-white/20 text-foreground/70 dark:text-white/70 hover:text-foreground dark:hover:text-white"
                    }`}
...
function DetailsCard({
  ...
  return (
    <div className="mt-8 rounded-xl overflow-hidden border border-foreground/15 dark:border-white/15 bg-foreground/[0.05] dark:bg-white/[0.05] shadow-[0_0_0_1px_rgba(255,255,255,0.04)]">
      <button
        id={buttonId}
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="w-full group flex items-center justify-between gap-4 px-4 py-3 md:px-5 md:py-4 focus:outline-none focus-visible:ring-2 focus-visible:ring-foreground/40 dark:focus-visible:ring-white/40 bg-foreground/5 dark:bg-white/5 hover:bg-foreground/10 dark:hover:bg-white/10 transition"
        aria-expanded={open}
        aria-controls={panelId}
      >
        <span className="font-mono text-sm tracking-wide text-foreground dark:text-white">
          {title}
        </span>
        <ChevronDown
          className={`h-4 w-4 text-foreground/90 dark:text-white/90 transition-transform duration-300 ${
            open ? "rotate-180" : ""
          }`}
          aria-hidden="true"
        />
      ...
      >
        <div className="min-h-0 overflow-hidden">
          <div className="px-4 py-4 md:px-5 md:py-5 bg-foreground/5 dark:bg-white/5 border-t border-foreground/10 dark:border-white/10">
            {children}
          </div>
        </div>
      </div>
