...
export function SustainabilitySection() {
  ...
  return (
    <section
      id="sustainability"
      className="min-h-screen flex items-center py-20 relative bg-background dark:bg-black"
    >
      <FloatingElement
        delay={1.5}
        amplitude={12}
      ...
      >
        <div className="w-3 h-3 bg-foreground dark:bg-white rounded-full" />
      </FloatingElement>

      <div className="max-w-4xl mx-auto px-4 sm:px-6 text-center relative z-10">
        <ScrollReveal delay={0.2}>
          <h2 className="text-2xl md:text-4xl font-mono text-foreground dark:text-white mb-4 tracking-wider">
        ...
        <ScrollReveal delay={0.4}>
          <div className="md:hidden grid grid-cols-1 gap-3">
            {currentSnippets.map((item, idx) => (
              <div
                key={idx}
                className="flex items-center gap-3 border border-foreground/15 dark:border-white/15 rounded-md p-3 bg-foreground/5 dark:bg-white/5"
              >
                <item.icon
                  className="h-5 w-5 text-foreground/80 dark:text-white/80"
                  aria-hidden={true}
                />
                <div className="flex-1 text-left">
                  <p className="font-mono text-sm text-foreground dark:text-white">{item.title}</p>
                  <p className="text-xs text-foreground/70 dark:text-white/70">{item.desc}</p>
                </div>
              </div>
            ))}
          </div>

          <div className="hidden md:block border border-foreground/20 dark:border-white/20 rounded-lg p-12 backdrop-blur-sm bg-foreground/5 dark:bg-white/5">
            <p className="font-mono text-base md:text-lg text-foreground/80 dark:text-white/80 leading-relaxed text-pretty">
